...
Student::Student(const Student& std) //복사생성자_ 깊은 복사가 일어나야함
{
	this->m_name = std.m_name;
	this->m_hakbun = std.m_hakbun;
	this->m_subnum = std.m_subnum;

	this->m_sub = new Subject[m_subnum]; // 새로운 공간을 할당하여, 재정의 
	//과목복사가 일어나야함.

	for (int i = 0; i < m_subnum; i++) // 멤버함수를 소유한 객체를 가리키는 포인터 this사용
	{
		(this->m_sub + i)->SetName((std.m_sub + i)->GetName()); // std의 내용을 그대로 베껴, 새로운 공간에 재현
...
string Student::GetName()const // 접근자함수 : 클래스 밖에서 멤버변수를 읽을 수 있게함
                               // 따로 값을 수정하지않기 때문에 const 멤버함수로 해줌
{
	return m_name;
}
